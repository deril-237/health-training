...
import { useGetInscriptionStatistics } from "../hooks";
import { BadgeStatistics } from "@/components/atoms/BadgeStatistics";

export function InscriptionStatisticsStatistics() {
  const { isFetching, data: statistics, error } = useGetInscriptionStatistics();
        ...
        <>
          <div>
            <BadgeStatistics
              variant="primary"
              title="Appcepté"
              subtitle={statistics?.inscriptionAccepted.toString() ?? ""}
            />
          </div>
          <div>
            <BadgeStatistics
              variant="warning"
              title="En cours"
              subtitle={statistics?.inscriptionSubmitted.toString() ?? ""}
            />
          </div>
          <div>
            <BadgeStatistics
              variant="error"
              title="Rejeté"
              subtitle={statistics?.inscriptionRejected.toString() ?? ""}
            />
          </div>
        </>
      )}
...
export function InscriptionStatisticsSkeleton() {
  return (
    <>
